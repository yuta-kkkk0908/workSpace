...
- `ニーズを分析`: 未分析ニーズの二次トリアージ（needs-triage）
...
make scenario-bot-post DATE=YYYY-MM-DD
make scenario-bot-sync LIMIT=100
make paper-exit-timing DATE=YYYY-MM-DD MODE=all
make paper-stats DATE=YYYY-MM-DD MODE=all
make inv-paper-stats-message DATE=YYYY-MM-DD
make trade-watch-review DATE=YYYY-MM-DD
```

ヘルス通知の推奨キーワード実行:

```bash
python scripts/ops/keyword_action.py 点検
python scripts/ops/keyword_action.py 点検補完
python scripts/ops/keyword_action.py 話題補完
python scripts/ops/keyword_action.py ニーズ補完
python scripts/ops/keyword_action.py 投資補完
python scripts/ops/keyword_action.py 全部
```
...
- `needs-triage`: `prompts/needs-triage.prompt.md` をCodexに貼って二次トリアージを実行
...
- `DISCORD_STATS_WEBHOOK_URL`（任意。未設定なら `DISCORD_SIGNAL_WEBHOOK_URL` へ投稿）
...
Scenario チャンネル運用:
- 投稿は `TRADE` 枠と `WATCH` 枠に分かれる。
- `scenario-bot-post` は短時間の同一銘柄・同一方向・同一tier再投稿を抑止（既定: 6時間）
...
- `exit [price]`
- `exit tp [price]` / `exit sl [price]` / `exit time` / `exit manual`
- `exit reason=tp price=4070`
- `cancel`
- `WATCH` 枠で `entry` した場合は `paper_trades.mode=watch` として保存され、検証用サンプルとして集計される。
