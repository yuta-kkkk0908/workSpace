...
### 投稿ラッパー（PowerShell）
- `scripts/run_inv_morning_and_post.ps1`
- `scripts/run_inv_noon_and_post.ps1`
- `scripts/run_inv_evening_and_post.ps1`
- `scripts/run_inv_scenario_and_post.ps1`
- `scripts/run_night_and_post_generic.ps1`
- `scripts/run_alert_and_post.ps1`
...
- `scripts/post_signal_discord.ps1`
  - `prompts/market-signals-discord-message.txt` を投稿
  - 同文スキップ運用あり（ハッシュ判定）
- `scripts/post_scenario_discord.ps1`
  - `prompts/opening-scenarios-discord-message.txt` を投稿
- `scripts/post_generic_discord.ps1`
  - `prompts/generic-topics-discord-message.txt` を投稿
- `scripts/post_alert_discord.ps1`
  - `prompts/pending-daily/latest.status.txt` を投稿

## 3. 投資パイプライン中核（Python）

### 収集/整形
- `scripts/prepare_morning_market_signals.py`
  - 前日シグナルを朝向けに軽量引き継ぎ
- `scripts/reevaluate_market_signals.py`
  - 夜間テクニカル確認 + ランク再評価
...
### シナリオ生成
- `scripts/load_rakuten_board_snapshot.py`
  - 楽天RSS CSVを `*-board-snapshot.json` 化
- `scripts/build_opening_scenarios.py`
  - 意思決定カード（トリガー/再現性/行動設計/見送り条件）作成
- `scripts/render_opening_scenarios_discord_message.py`
  - Discord投稿文を生成
...
### 検証（仮想トレード）
- `scripts/register_paper_trades.py`
...
- `scripts/ingest_topics_db.py`
- `scripts/build_today_topics_brief_from_db.py`
- `scripts/render_generic_topics_discord_message.py`
- `scripts/init_needs_db.py`
- `scripts/ingest_needs_db.py`
...
- `scripts/check_daily_missing_toast.ps1`
  - Windows通知 + クリップボード補助
- `scripts/run_alert_healthcheck.ps1`
  - 欠損チェックの定期実行用
...
# 朝シナリオ + 投稿
powershell -NoProfile -ExecutionPolicy Bypass -File E:\workSpace\scripts\run_inv_scenario_and_post.ps1

# 夜バッチ + 汎用トピック投稿
powershell -NoProfile -ExecutionPolicy Bypass -File E:\workSpace\scripts\run_night_and_post_generic.ps1
