...
function Invoke-AiosPostStep {
  ...
  if ($stepRc -ne 0) {
    ...
    Write-AiosPipelineEvent -Repo $repo -Pipeline "ops_post" -Slot "inv-noon" -Stage $Stage -Status "error" -ReturnCode $stepRc -EventDate $eventDate -Category $cat
  } else {
    Write-AiosPipelineEvent -Repo $repo -Pipeline "ops_post" -Slot "inv-noon" -Stage $Stage -Status "ok" -ReturnCode 0 -EventDate $eventDate -Category "ok"
  }
  return $stepRc
}

& $python "scripts/run_ops_scheduler.py" --slot inv-noon --date $eventDate
$rc = $LASTEXITCODE
if ($rc -eq 0 -and -not $isWeekend) {
  ...
  Write-Host "[skip] weekend post steps: inv-noon $eventDate"
}
exit $rc
