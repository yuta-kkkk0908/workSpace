...
$statusPath = Join-Path $RepoPath "tmp\prompts\pending-daily\latest.status.txt"
$promptPath = Join-Path $RepoPath "tmp\prompts\pending-daily\latest.prompt.md"
$clipboardPath = Join-Path $RepoPath "tmp\prompts\pending-daily\latest.clipboard.txt"
...
if (-not (Test-Path "scripts\monitoring\check_daily_missing.py")) {
    Write-ReminderLog "missing script at $(Join-Path (Get-Location) 'scripts\monitoring\check_daily_missing.py')"
    exit 2
}

$output = & python "scripts\monitoring\check_daily_missing.py" --date $Date --days $Days 2>&1
