...
PENDING_DIR = ROOT / "tmp" / "prompts" / "pending-daily"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to write the latest generated prompt. Default: tmp/prompts/pending-daily/latest.prompt.md",
    )
    parser.add_argument(
        "--status-path",
        default=str(STATUS_PATH.relative_to(ROOT)),
        help="Path to write the latest short notification status. Default: tmp/prompts/pending-daily/latest.status.txt",
    )
    parser.add_argument(
        "--days",
        type=int,
...
def expected_files(target: date) -> list[Path]:
    ...
    files = [
        TOPICS_DIR / topic / "source" / f"{date_str}-daily.md"
